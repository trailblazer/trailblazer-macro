...
# test the old style of Wrap()
class DeprecatedWrapTest < Minitest::Spec
...
# it allows returning legacy true/false
  ...
  it { Create.( params: {id: 1, title: "Prodigal Son"} )["contract.default"].model.inspect.must_equal %{#<struct DeprecatedWrapTest::Song id=1, title="Prodigal Son">} }
  ...
  #:sequel-transaction end
    ...
    it { Create.( params: {title: "Pie"} ).inspect(:model, "x", "err").must_equal %{<Result:true [#<struct DeprecatedWrapTest::Song id=nil, title=\"Pie\">, nil, nil] >} }
  end
